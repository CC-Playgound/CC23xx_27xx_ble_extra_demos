...
pattern = re.compile(r"Central RSSI = (-?\d+)")

data = {}  # dist -> list of RSSI values

for dist in range(1, 11):
    log_file = log_dir / f"{dist}m_cm.log"
...
fig1.tight_layout()
fig1.savefig(log_dir / "central_rssi_timeseries.png", dpi=150)
...
fig2.tight_layout()
fig2.savefig(log_dir / "central_rssi_by_distance.png", dpi=150)
...
ax3.set_title("CM RSSI All Samples by Distance")
ax3.legend(title="Distance", loc="lower right", markerscale=3)
ax3.grid(True, axis="y", alpha=0.3)
fig3.tight_layout()
fig3.savefig(log_dir / "central_rssi_scatter.png", dpi=150)
